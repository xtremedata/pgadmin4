define('pgadmin.node.table', [
  ...
  'pgadmin.alertify', 'pgadmin.browser.collection', 'pgadmin.node.column',
  'pgadmin.node.constraints', 'pgadmin.browser.table.partition.utils',
], function(
  gettext, url_for, $, _, Backform, Backgrid, pgBrowser, Alertify
) {

  if (!pgBrowser.Nodes['coll-table']) {
    pgBrowser.Nodes['coll-table'] = pgBrowser.Collection.extend({
      node: 'table', label: gettext('Tables'), type: 'coll-table',
      columns: ['name', 'relowner', 'is_partitioned', 'description'],
      statsPrettifyFields: [
        'Size', 'Indexes size', 'Table size', 'Toast table size',
        'Tuple length', 'Dead tuple length', 'Free space',
      ], hasStatistics: true,
    });
  }
    ...
    pgBrowser.Nodes.table = pgBrowser.Node.extend({
      ...
      callbacks: {
        ...
        truncate: function(args, params) {
          ...
          Alertify.confirm(
            ...
            gettext('Are you sure you want to truncate table %(label)s?', {label: d._label}),
            function (e) {
              if (e) {
                var data = d;
                $.ajax({
        ...
        reset_table_stats: function(args) {
          ...
          Alertify.confirm(
            ...
            gettext(
              'Are you sure you want to reset the statistics for table %(label)s?', {
                label: d._label,
              }),
            function (e) {
              if (e) {
      ...
      model: pgBrowser.Node.Model.extend({
        ...
        schema: [{
            ...
            onChange: function() {
              ...
              if (!_.isUndefined(tbl_name) && !_.isNull(tbl_name) &&
                ...
                Alertify.confirm(title, msg, function () {
                  ...
                  setTimeout(
                      function() { self.model.set('typname', null); }, 10
                    );
                });
          ...
          visible: function(m) {
            var server = m.node_info && m.node_info.server;
            return  server && server.version && server.version >= 100000;
          },
          disabled: function(m) {
            if (
        ...
        if (
          ...
          if ('detached' in partitions && partitions.detached.length > 0) {
            ...
            var detachedBySchema = _.groupBy(
              ...
              schemaNode = pgBrowser.findParentTreeNodeByType(_node, 'schema');
              ...
              if (schemaNode) {
                ...
                if (tablesCollNode) {
                  insertChildTreeNodes.push({
                    ...
                    'type': 'table',
                    'treeHierarchy': pgBrowser.Nodes.schema.getTreeNodeHierarchy(schemaNode),
