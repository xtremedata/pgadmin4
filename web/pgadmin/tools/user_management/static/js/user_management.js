define([
  ...
  'underscore.string', 'pgadmin.alertify', 'pgadmin.browser',
    ...
    show_users: function() {
      ...
      var UserModel = pgBrowser.Node.Model.extend({
          ...
          validate: function() {
            ...
            if (
              ('email' in changedAttrs || !this.isNew()) && (
                _.isUndefined(this.get('email')) ||
              ...
              !!this.get('email') && !email_filter.test(this.get('email'))
            ) {
              errmsg = gettext('Invalid email address: %(email)s.', {
              ...
              !!this.get('email') &&
                this.collection.where({'email':this.get('email')}).length > 1
            ) {
              errmsg =  gettext('The email address %(email)s already exists.', {
                email: this.get('email'),
              });
              this.errorModel.set('email', errmsg);
              return errmsg;
            ...
            if (
              'role' in changedAttrs && (
                _.isUndefined(this.get('role')) ||
                  _.isNull(this.get('role')) ||
                  String(this.get('role')).replace(/^\s+|\s+$/g, '') === ''
              )
            ) {
              errmsg =  gettext('Role cannot be empty for user %(email)s', {
                email: (this.get('email') || ''),
              });
            ...
            if(this.isNew()) {
              ...
              if (
                'newPassword' in changedAttrs && (
                  _.isUndefined(this.get('newPassword')) ||
                    _.isNull(this.get('newPassword')) ||
                    this.get('newPassword') === ''
                )
              ) {
                errmsg = gettext(
                  'Password cannot be empty for user %(email)s.', {
                    email: (this.get('email') || ''),
                  });
                ...
                errmsg = gettext(
                  'Password must be at least 6 characters for user %(email)s.', {
                    email: (this.get('email') || ''),
                  });
                this.errorModel.set('newPassword', errmsg);
                return errmsg;
              ...
              if (
                'confirmPassword' in changedAttrs && (
                  _.isUndefined(this.get('confirmPassword')) ||
                    _.isNull(this.get('confirmPassword')) ||
                    this.get('confirmPassword') === ''
                )
              ) {
                errmsg =  gettext(
                  'Confirm Password cannot be empty for user %(email)s.', {
                    email: (this.get('email') || ''),
                  });
              ...
              if (
                ...
                errmsg = gettext('Passwords do not match for user %(email)s.', {
                  email: (this.get('email') || ''),
                });
                this.errorModel.set('confirmPassword', errmsg);
                return errmsg;
              ...
              if ((
                ...
                errmsg = gettext(
                  'Passwords do not match for user %(email)s.', {
                    email: (this.get('email') || ''),
                  });
                this.errorModel.set('confirmPassword', errmsg);
                return errmsg;
                ...
                alertify.confirm(
                    ...
                    function() {
                      self.model.destroy({
                        ...
                        error: function() {
                          alertify.error(
                            gettext('Error occurred during deleting user.')
                          );
                        },
                      });
                    },
              ...
              var $footer = $([
                ...
                UserCollection = Backbone.Collection.extend({
                  ...
                  saveUser: function(m) {
                    ...
                    if(
                      m.isNew() && (!m.get('email') || !m.get('role') ||
                    ...
                    if (m.sessChanged() && d && !_.isEmpty(d)) {
                      ...
                      m.save({}, {
                        ...
                        wait: true,
                        success: function() {
                          // User created/updated on server now start new
