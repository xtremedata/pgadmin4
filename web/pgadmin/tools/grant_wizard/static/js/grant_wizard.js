...
define([
  ...
  'pgadmin.alertify', 'backbone', 'pgadmin.backgrid', 'pgadmin.backform',
  'pgadmin.browser', 'pgadmin.browser.tool', 'pgadmin.browser.node',
  'backgrid.select.all', 'backgrid.filter', 'pgadmin.browser.server.privilege',
  'pgadmin.browser.wizard',
], function(
  gettext, url_for, $, _, Alertify, pgBrowser, pgTool, Backbone, Backgrid, Backform, pgNode
) {
  ...
  /*
   * It is a sub model for field "Objects". It has fields for database object
   * types such as Schemas, Views and Sequence etc.
   */
  ...
  var columns = [{
    ...
    /*
     * Override render method of Backgrid.Extension.SelectRowCell class. It
     * has an issue: It doesn't mark rows checked if we move to next page
     * and then go back to previous page. but it must show. so we handle
     * this case by overriding the render method.
     */
  ...
  pgBrowser.GrantWizard = pgTool.extend({
    Init: function() {
      ...
      var supported_nodes = [
        ...
        /*
         * Enable/disable grantwizard menu in tools based on node
         * selected. If selected node is present in supported_nodes, menu
         * will be enabled otherwise disabled.
         * Also, hide it for system view in catalogs.
         */
        menu_enabled = function(itemData, item) {
          var t = pgBrowser.tree, i = item, d = itemData,
            parent_item = t && t.hasParent(i) ? t.parent(i): null,
            parent_data = parent_item ? t.itemData(parent_item) : null;

          if (
                !_.isUndefined(d) && !_.isNull(d) && !_.isNull(parent_data)
              ) {
            if (
                  _.indexOf(supported_nodes, d._type) !== -1 &&
      ...
      if (!Alertify.wizardDialog) {
        Alertify.dialog('wizardDialog', function factory() {
          ...
          return {
            ...
            /*
             * Callback called when an errorModel is set with invalid value and
             * errormsg is set into status bar element and next button is
             * disabled.
             */
            ...
            /*
             * Callback called when anything is set into model thus hide error
             * msg element and enable next button status bar element and next
             * button is disabled.
             */
            ...
            /*
             * Remove/Delete objects, attributes in wizard on wizard close or
             * finish to reclaim memory.
             */
            ...
            /*
             * Every time a wizard is opened, this function is called
             * everytime. It has Wizard Pages which are rendered by the
             * Wizard Class:
             *
             * @class {pgBrowser.WizardPage} dbObjectType1
             * @extends {Backbone.Model}
             * - This page renders a grid of Database Object Types such as
             *   Schemas, Views and Sequences etc.
             *
             * @class {pgBrowser.WizardPage} WizardPage2
             * @extends {Backbone.Model}
             * - This page adds Privilege Control which provides grant
             *   privileges such as "Create, Insert, Delete, Update" so on the
             *   database objects selected on Wizard Pages.
             *
             * @class {pgBrowser.WizardPage} WizardPage3
             * @extends {Backbone.Model}
             * - This page displays the generated GRANT SQL query for the Db
             *   objects selected with the specific privileges added to it.
             *
             * @class {Backbone.Collection} WizardCollection
             * - It is the collection of wizard pages
             *
             * @class {pgBrowser.Wizard} wizard
            ...
            prepare:function() {
              ...
              $container.empty().append(
                '<div class=\'grant_wizard_container\'></div>'
              );
              ...
              /*
               * Generate a URL using: gid, did, sid(server id),
               * node_id(node id), node_(node name), node_type(node type),
               * and pass it to collection which will fetch Object Type
               * properties.
               */
              var gid = info['server_group']._id,
                ...
                node_id = d._id,
                /*
                 * Get node name only. used in mapping with object types
                 * defined in allowed_acl.json
                 */
              ...
              // Collection url to fetch database object types for objects
              // field.
              ...
              this.msql_url = url_for('grant_wizard.modified_sql', {
                'sid': encodeURI(sid), 'did': encodeURI(did),
              });
              ...
              /*
               * privArray holds objects selected which further helps in
               * creating privileges Model.
               */
              self.privArray = [];

              /*
               * Override backgrid listener "backgrid:selected" to Add/Remove
               * model to/from objects collection.
               */
              coll.on('backgrid:selected', function(model, selected) {
                ...
                /*
                 * If a row (checkbox) is checked, add that model into
                 * collection, when unchecked remove it from model.
                 *
              ...
              /*
               * It is the main model with schema defined. Every time a new
               * wizard is opened, a new model should create.
               */
              var GrantWizardModel = pgNode.Model.extend({
                defaults: {
                  objects: undefined, acl: undefined,
                },
                schema: [{
                  id: 'objects', label: gettext('Objects'),
                  model: DatabaseObjectModel, type: 'collection',
              ...
              /*
               * Create instance of GrantWizard Model, provide the urlRoot
               * node_info object, Generate fields objects.
               */
              var newModel = new GrantWizardModel({}, { node_info: info });

              /*
               * Fetch data from server and set into grid and show/hide
               * progress bar.
               */
              ...
              /*
               * Create the wizard page. It renders a grid of Database Object
               * Types such as Schemas, Views and Sequences etc. Set the
               * default values.
               */
                ...
                view: new (function() {
                  ...
                  _.extend(pageView, {
                    ...
                    render: function() {
                      ...
                      /*
                       * Fetch selected models from collection and make rows
                       * checked in grid.
                       */
              ...
              var privilegePage = self.privilegePage = new pgBrowser.WizardPage({
                ...
                view: new (function() {
                  ...
                  _.extend(pageView, {
                    ...
                    render: function() {
                      ...
                      /*
                       * Define PrivModel and its instance. Privileges array
                       * is generated based on the type of nodes selected.
                       */
                      ...
                      /*
                       * To track changes into model, start new session and Add
                       * event listener for privileges control
                       */
                ...
                beforePrev: function(wizardObj) {
                  ...
                  /*
                   * Enable/Disable next button of DbObjectType page if
                   * objects are present in model.
                   */
                  var objectsModel = newModel.get('objects');

                  if(
                    !_.isUndefined(objectsModel) && !_.isEmpty(objectsModel) &&
                   ...
                   * before heading to next page Refresh Backgrid to re-render
                   * the elements selected re-render Filter.
                   */
              ...
              var sqlField = new Backform.Field({
                  ...
                  /*
                   * Extend 'SqlTabControl' to define new function
                   * 'onWizardNextPageChange' which gets triggered on next
                   * button click to fetch generated SQL query for the selected
                   * db objects.
                   */
              ...
              var reviewSQLPage = self.reviewSQLPage = new pgBrowser.WizardPage({
                ...
                beforePrev: function(wizardObj) {
                  /*
                   * Enable 'next' button if privilege model is not empty else
                   * disable the 'next' button.
                   */
                  var aclModel = newModel.get('acl');

                  if(
                    !_.isUndefined(wizardObj.collection) &&
                    wizardObj.collection.models.length > 0
                  ) {
                    if(
                      !_.isUndefined(aclModel) && !_.isEmpty(aclModel) &&
                        aclModel.length > 0
                    ) {
                  ...
                  if (d && !_.isEmpty(d) && !_.isUndefined(d.objects)) {
                    m.save({}, {
                      ...
                      error: function(m, jqxhr) {
                        Alertify.pgNotifier('error', jqxhr,
                          gettext('Error saving properties: %(message)s', {
