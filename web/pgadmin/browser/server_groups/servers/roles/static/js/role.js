define('pgadmin.node.role', [
  ...
  'underscore.string', 'pgadmin.browser', 'pgadmin.backform', 'select2',
  'pgadmin.browser.collection', 'pgadmin.browser.node.ui',
  'pgadmin.browser.server.variable',
], function(gettext, url_for, $, _, S, pgBrowser, Backform) {
  ...
  if (!pgBrowser.Nodes['coll-role']) {
    pgBrowser.Nodes['coll-role'] = pgBrowser.Collection.extend({
      node: 'role', type: 'coll-role',
      columns: [
  ...
  var SecurityModel = pgBrowser.Node.Model.extend({
    defaults: {
      provider: null,
      label: null,
    },
  ...
  pgBrowser.SecurityModel = SecurityModel;
  ...
  var RoleMembersControl = Backform.Control.extend({
    ...
    formatter: {
      fromRaw: function (rawData) {
        return _.pluck(
          _.isObject(rawData) ? rawData : JSON.parse(rawData), 'role'
        );
      },
  ...
  pgBrowser.Nodes.role = pgBrowser.Node.extend({
    parent_type: 'server',
    type: 'role',
    ...
    model: pgBrowser.Node.Model.extend({
      ...
      schema: [{
        ...
        control: 'variable-collection', disabled: 'readonly', canAdd: true,
