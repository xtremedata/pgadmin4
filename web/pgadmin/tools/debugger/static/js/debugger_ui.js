define([
  ...
  return function(args, restart_debug) {
        ...
        return {
          main:function(title, data, restart_debug) {
            ...
            $.ajax({
              ...
              error: function() {
                Alertify.alert(gettext('Error setting the arguments'));
              },
            });
            ...
            var input_arg_id = this.input_arg_id = [], j, k;
            ...
            if (this.data['proargnames'] != null) {
              ...
              if (func_args_data.length != 0) {
                for (i = 0; i < func_args_data.length; i++) {
                  ...
                  func_obj.push({
                    'name': argname[index], 'type': argtype[index],
                    'is_null': !!func_args_data[i]['is_null'],
                    'expr': !!func_args_data[i]['is_expression'],
                    'value': values,
                    'use_default': !!func_args_data[i]['use_default'],
              ...
              /*
               * Generate the name parameter if function do not have arguments
               * name like dbgparam1, dbgparam2 etc.
               */
              var myargname = [];
              ...
              if (!this.data['pronargdefaults']) {
                ...
                for (i = 0; i < argtype.length; i++) {
                  if (this.data['proargmodes'] == null) {
                    ...
                    my_obj.push({
                      'name': myargname[i], 'type': argtype[i],
                      'use_default': use_def_value,
                      'default_value': def_val_list[i],
                    });
                  } else {
              ...
              if (func_args_data.length != 0) {
                for (i = 0; i < func_args_data.length; i++) {
                  ...
                  func_obj.push({
                    'name': myargname[index], 'type': argtype[index],
                    'is_null': !!func_args_data[i]['is_null'], 'value': values,
                    'expr': !!func_args_data[i]['is_expression'],
                    'use_default': !!func_args_data[i]['use_default'],
                    'default_value': def_val_list[index],
                  });
            ...
            this.debuggerInputArgsColl =
              new DebuggerInputArgCollections(
                func_args_data.length == 0 ? my_obj : func_obj
              );
            ...
            if (e.button.text === 'Debug') {
              ...
              if (self.restart_debug == 0) {
                ...
                $.ajax({
                  ...
                  error: function(e) {
                    Alertify.alert(
                      gettext('Error starting the debugger listener'),
                      e.responseJSON.errormsg
                    );
                  },
                });
          ...
          prepare:function() {
             ...
             * should enable the debug button otherwise disable the debug
             * button.
             */
            ...
            /*
             * Listen to the grid change event so that if any value changed by
             * user then we can enable/disable the debug button.
             */
