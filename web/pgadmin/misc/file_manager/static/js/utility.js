    ...
    getNewName = function(rname) {
      if (rname !== '') {
        ...
        $.ajax({
          type: 'POST',
          data: JSON.stringify({
            'mode': 'rename', 'old': data.Path, 'new': givenName,
          }),
...
var deleteItem = function(data) {
  var isDeleted = false,
    ...
    doDelete = function(data) {
      $.ajax({
        type: 'POST',
        data: JSON.stringify({'mode': 'delete', 'path': data.Path}),
        url: pgAdmin.FileUtils.fileConnector,
        dataType: 'json',
        contentType: 'application/json; charset=utf-8',
        async: false,
...
var getFileInfo = function(file) {
  ...
  $.ajax({
    type: 'POST',
    data: JSON.stringify({'path': file, 'mode': 'getinfo'}),
    url: pgAdmin.FileUtils.fileConnector,
    dataType: 'json',
    contentType: 'application/json; charset=utf-8',
    async: false,
...
var checkPermission = function(path) {
  ...
  $.ajax({
    type: 'POST',
    data: JSON.stringify({'path': path, 'mode': 'permission'}),
    url: pgAdmin.FileUtils.fileConnector,
    dataType: 'json',
    contentType: 'application/json; charset=utf-8',
    async: false,
...
var getFolderInfo = function(path, file_type) {
  ...
  $.ajax({
    type: 'POST',
    data: JSON.stringify({
      'path': path, 'mode': 'getfolder', 'file_type': file_type || '*',
      ...
      $('.fileinfo #contents li p').on('blur dblclick','input', function(e) {
        ...
        if (e.type=='keydown') {
          ...
          if ($(this).css('display')=='inline-block' || $(this).css('display')=='inline') {
            full_name = decodeURI($(this).val()) + (last !== ''? '.' + last : '');
...
pgAdmin.FileUtils = {
  init: function() {
    ...
    $('.file_manager .refresh').on('click', function() {
      enable_disable_btn();
      var curr_path = $('.currentpath').val(), path;
...
module.exports = pgAdmin.FileUtils;
