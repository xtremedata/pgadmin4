define([
  ...
  pgTools.Debugger = pgTool.extend({
    ...
    can_debug: function(itemData, item, data) {
      ...
      if(
        data && data.debug_type && data.debug_type == 'indirect' &&
    ...
    generate_url: function(_url, treeInfo, node) {
      ...
      _.each(
        _.sortBy(
          _.values(_.pick(
            treeInfo, function(v, k) { return k !== 'server_group'; }
          )),
    ...
    start_global_debugger: function() {
      ...
      var treeInfo = node.getTreeNodeHierarchy.apply(node, [i]), baseUrl;
    ...
    /*
     * Get the function information for the direct debugging to display the
     * functions arguments and other informations in the user input dialog.
     */
