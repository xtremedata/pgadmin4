...
], function(gettext, url_for, $, _, S, Backform, pgBrowser) {

  // Extend the browser's collection class for event trigger collection
  if (!pgBrowser.Nodes['coll-event_trigger']) {
    pgBrowser.Nodes['coll-event_trigger'] = pgBrowser.Collection.extend({
      node: 'event_trigger', label: gettext('Event Trigger'),
      type: 'coll-event_trigger', columns: ['name', 'eventowner', 'comment'],
    });
  }

  // Extend the browser's node class for event triggers node
  if (!pgBrowser.Nodes.event_trigger) {
    pgBrowser.Nodes.event_trigger = pgBrowser.Node.extend({
