define('pgadmin.datagrid', [
  ...
  pgAdmin.DataGrid = pgTool.extend({
    ...
    show_data_grid: function(data, i) {
      ...
      if (d === undefined) {
        Alertify.alert(
          gettext('Data Grid Error'), gettext('No object selected.')
        );
        return;
      }
                ...
                $.ajax({
                  ...
                  success: function(res) {
                    if (res.data.status) {
                      ...
                      Alertify.alert(
                        gettext('Validation Error'), res.data.result
                      );
                    }
                  },
                  error: function(e) {
                    Alertify.alert(gettext('Validation Error'), e);
                  },
                });
              }
            },
      ...
      $.get(url_for('datagrid.filter'),
        function(data) {
          Alertify.filterDialog(
            gettext('Data Filter'), data, baseUrl, validateUrl
          ).resizeTo(600, 400);
        }
      );
    },
    ...
    initialize_data_grid: function(baseUrl, grid_title, sql_filter, server_type) {
      ...
      $.ajax({
        ...
        error: function() {
          Alertify.alert(gettext('Error initializng the Query tool'));
        },
      });
    },
    ...
    show_query_tool: function(url, i, panel_title) {
      ...
      if (d === undefined) {
        Alertify.alert(
          gettext('Query tool Error'), gettext('No object selected.')
        );
        return;
      }
