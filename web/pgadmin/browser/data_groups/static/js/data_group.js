...
define('pgadmin.node.data_group', [
  'sources/gettext', 'sources/url_for', 'jquery', 'underscore',
  'sources/pgadmin', 'pgadmin.browser', 'pgadmin.browser.node',
], function(gettext, url_for, $, _, pgAdmin) {

  if (!pgAdmin.Browser.Nodes['data_group']) {
    pgAdmin.Browser.Nodes['data_group'] = pgAdmin.Browser.Node.extend({
      parent_type: null,
      type: 'data_group',
      dialogHelp: url_for('help.static', {'filename': 'data_group_dialog.html'}),
      ...
      Init: function() {
        ...
        pgAdmin.Browser.add_menus([{
          name: 'create_data_group', node: 'data_group', module: this,
          applies: ['object', 'context'], callback: 'show_obj_properties',
          category: 'create', priority: 1, label: gettext('Data Group...'),
          data: {'action': 'create'}, icon: 'wcTabIcon icon-data_group',
        }]);
      },
      ...
      canDelete: function(i) {
        ...
        /* This is the only data group - we can't remove it*/
        if (!s || s.length == 0) {
          return false;
        }
        return true;
  ...
  return pgAdmin.Browser.Nodes['data_group'];
});
