define('pgadmin.node.type', [
  ...
  'underscore.string', 'pgadmin.backform', 'pgadmin.backgrid',
  'pgadmin.browser', 'pgadmin.browser.collection',
], function(gettext, url_for, $, _, S, Backform, Backgrid, pgBrowser) {

  if (pgBrowser.Nodes.type) {
    return pgBrowser.Nodes.type;
  }

  pgBrowser.Nodes['coll-type'] = pgBrowser.Collection.extend({
    node: 'type', label: gettext('Types'), type: 'coll-type',
  ...
  var CompositeModel = Backform.CompositeModel = pgBrowser.Node.Model.extend({
    ...
    validate: function() {
      ...
      if (
        _.isUndefined(this.get('member_name')) ||
      ...
      if (
        _.isUndefined(this.get('type')) || _.isNull(this.get('type')) ||
      ...
      if (this.get('is_tlength') && !_.isUndefined(this.get('tlength'))) {
      ...
      if (this.get('is_precision') && !_.isUndefined(this.get('precision'))) {
        var val = this.get('precision');
        if (val < this.get('min_val')) {
          errmsg = gettext('Scale should not be less than  %(value)s', {
            value: this.get('min_val'),
          });
        } else if (val > this.get('max_val')) {
          errmsg = gettext('Scale should not be greater than  %(value)s', {
            value: this.get('max_val'),
          });
        }
        // If we have any error set then throw it to user
        if(errmsg) {
