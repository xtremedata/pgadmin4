define('pgadmin.node.user_mapping', [
  ...
  if (!pgBrowser.Nodes['coll-user_mapping']) {
    pgBrowser.Nodes['coll-user_mapping'] = pgBrowser.Collection.extend({
      node: 'user_mapping', label: gettext('User Mappings'),
      type: 'coll-user_mapping', columns: ['name'],
    });
  }
  ...
  if (!pgBrowser.Nodes.user_mapping) {
    ...
    pgBrowser.Nodes.user_mapping = pgBrowser.Node.extend({
