define('pgadmin.node.trigger', [
  ...
  'pgadmin.browser', 'pgadmin.backform', 'pgadmin.alertify',
  'pgadmin.browser.collection', 'pgadmin.node.schema',
], function(gettext, url_for, $, _, pgBrowser, Backform, alertify) {
  ...
  if (!pgBrowser.Nodes['coll-trigger']) {
    pgBrowser.Nodes['coll-trigger'] = pgBrowser.Collection.extend({
      node: 'trigger', label: gettext('Triggers'), type: 'coll-trigger',
      getTreeNodeHierarchy: pgBrowser.tableChildTreeNodeHierarchy,
      columns: ['name', 'description'],
    });
  }

  pgBrowser.Nodes.trigger = pgBrowser.Node.extend({
    getTreeNodeHierarchy: pgBrowser.tableChildTreeNodeHierarchy,
      ...
      schema: [{
        ...
        disabled: function(m) {
          ...
          if (
            ...
            setTimeout(
              function() { m.set('is_constraint_trigger', false); },10
            );
