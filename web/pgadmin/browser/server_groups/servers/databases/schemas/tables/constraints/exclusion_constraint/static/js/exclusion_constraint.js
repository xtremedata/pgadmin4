define('pgadmin.node.exclusion_constraint', [
  ...
  'pgadmin.alertify', 'pgadmin.browser.collection',
], function(
  gettext, url_for, $, _, Backbone, Backform, Backgrid, pgBrowser, Alertify
) {
    ...
    schema: [{
      ...
      select2: {
        ...
        initialize: function () {
          ...
          if (
            url && (
              _.isUndefined(indextype) ||  _.isNull(indextype) ||
  ...
  var ExclusionConstraintColumnControl =
  ...
  Backform.ExclusionConstraintColumnControl = ExclusionConstraintColumnControl;

  // Extend the browser's node class for exclusion constraint node
  pgBrowser.Nodes.exclusion_constraint = pgBrowser.Node.extend({
    ...
    model: pgBrowser.Node.Model.extend({
      ...
      validate: function() {
        ...
        if ((_.isUndefined(name) || _.isNull(name) || name.length < 1)) {
          ...
          _.isUndefined(columns) || _.isNull(columns) ||
            columns.length < 1
        ) {
