define(['sources/gettext', 'underscore', 'jquery', 'backbone', 'backform',
  'backgrid', 'pgadmin.browser.node', 'pgadmin.browser.node.ui',
],
 function(gettext, _, $, Backbone, Backform, Backgrid, pgNode) {
  /**
   ...
   **/
   var PrivilegeRoleModel = pgNode.Model.extend({
     idAttribute: 'grantee',
     defaults: {
       grantee: undefined,
       grantor: undefined,
       ...
       cell: Backgrid.Extension.NodeListByNameCell.extend({
         initialize: function(opts) {
          ...
          // Let's override the options
           if (override_opts) {
             var options = self.column.get('options');
             self.column.set(
               'options', self.omit_selected_roles.bind(self, options)
             );
           }
   ...
   pgNode.PrivilegeRoleModel = PrivilegeRoleModel;
