define('pgadmin.node.fts_configuration', [
...
], function(gettext, url_for, $, _, Backbone, Backform, Backgrid, pgBrowser) {

  // Extend the collection class for FTS Configuration
  if (!pgBrowser.Nodes['coll-fts_configuration']) {
    pgBrowser.Nodes['coll-fts_configuration'] =
      pgBrowser.Collection.extend({
        node: 'fts_configuration', label: gettext('FTS Configurations'),
        type: 'coll-fts_configuration', columns: ['name', 'description'],
      });
  }
  ...
  if (!pgBrowser.Nodes.fts_configuration) {
    ...
    var TokenModel = pgBrowser.Node.Model.extend({
      defaults: {
        token: undefined,
        dictname: undefined,
      },
    ...
    var TokenControl =  Backform.UniqueColCollectionControl.extend({
            ...
            control: Backform.NodeAjaxOptionsControl.extend({
              ...
              initialize: function() {
                ...
                if (url) {
                  ...
                  cache_node = (cache_node && pgBrowser.Nodes['cache_node']) ||
                    node;
                  ...
                  if (this.field.get('version_compatible') &&
                    ...
                    $.ajax({
                      ...
                      success: function(res) {
                         ...
                         * avoiding same calls.
                         */
                        data = cache_node.cache(
                          url, node_info, cache_level, res.data
                        );
                      },
                      error: function() {
      ...
      headerDataChanged: function() {
        var data = this.headerData.toJSON(),
        ...
        this.$header.find('button.add').prop('disabled', inSelected);
      },
    ...
    pgBrowser.Nodes['fts_configuration'] = pgBrowser.Node.extend({
