define('pgadmin.browser', [
  ...
  'underscore.string', 'bootstrap', 'sources/pgadmin', 'pgadmin.alertify',
  ...
  _.extend(pgAdmin.Browser, {
    ...
    get_preference: function(module, preference) {
      ...
      if(_.size(self.preferences_cache) == 0) {
        var preference_timeout,
          check_preference = function () {
            if(_.size(self.preferences_cache) > 0) {
              clearInterval(preference_timeout);
              return _.findWhere(
                self.preferences_cache, {'module': module, 'name': preference}
              );
            }
          };
        preference_timeout = setInterval(check_preference, 1000);
      } else {
        return _.findWhere(
          self.preferences_cache, {'module': module, 'name': preference}
        );
            ...
            addNode = function() {
              ...
              if (binarySearch()) {
                ctx.t.before(i, {
                  ...
                  fail: function() {
                    console.warn('Failed to add before..');
                    if (
            ...
            if(_parent.length == 0 && ctx.op == 'UPDATE') {
              ...
              var postRemove = function() {
                ...
                if (_item_parent && !_item_grand_parent) {
                  var parent = null;
                  // We need to search in all parent siblings
                  // eg. server groups
                  ...
                  if (parent) {
                    ...
                    this.success = function() { addItemNode(); }.bind(this);
        ...
        $.ajax({
          ...
          error: function(xhr, error, status) {
            ...
            !Alertify.pgHandleItemError(
              ...
              Alertify.pgNotifier(
              error, xhr,
              gettext('Error retrieving details for the node.'),
              function() { }
            );
            }
          },
    ...
    removeChildTreeNodesById: function(_parentNode, _collType, _childIds) {
      var tree = pgBrowser.tree, idx, children,
        childNode, childNodeData, size;
