define('pgadmin.node.cast', [
  ...
  gettext, url_for, $, _, S, pgAdmin, Backform, pgBrowser, alertify
) {
  // Extend the collection class for cast
  if (!pgBrowser.Nodes['coll-cast']) {
    pgAdmin.Browser.Nodes['coll-cast'] = pgAdmin.Browser.Collection.extend({
      node: 'cast', label: gettext('Casts'), type: 'coll-cast',
  ...
  if (!pgBrowser.Nodes.cast) {
    pgBrowser.Nodes.cast = pgBrowser.Node.extend({
          ...
          control: Backform.NodeAjaxOptionsControl.extend({
            ...
            onChange: function() {
              Backform.NodeAjaxOptionsControl.prototype.onChange.apply(
                   ...
                   */
              var srcType = this.model.get('srctyp'),
                trgtype = this.model.get('trgtyp');
              if(
                  srcType != undefined && srcType != '' &&
                    trgtype != undefined && trgtype != ''
                ) {
                this.model.set('name', srcType+'->'+trgtype);
              } else {
