define([
  ...
  var ImportExportModel = Backbone.Model.extend({
    ...
    importing: function(m) { return m.get('is_import'); },
    exporting: function(m) { return !(m.importing.apply(this, arguments)); },
  });
  ...
  pgTools.import_utility = pgTool.extend({
    ...
    callback_import_export: function(args, item) {
        ...
        Alertify.dialog('ImportDialog', function factory() {
          ...
          return {
            ...
            prepare: function() {
              ...
              // Listen to model & if filename is provided then enable the 'OK'
              // button.
              //
              // For the 'Quote', 'escape' and 'delimiter' only one character
              // is allowed to enter
      ...
      Alertify.ImportDialog(
        gettext('Import/Export data - table \'%(label)s\'', {
          label: treeInfo.table._label,
        }), node, i, d
