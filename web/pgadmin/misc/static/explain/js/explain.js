    ...
    draw: function(s, xpos, ypos, pXpos, pYpos, graphContainer, toolTipContainer) {
      ...
      if (!_.isUndefined(pYpos)) {
        ...
        if(start_cost != undefined && total_cost != undefined) {
          arrow_size = Math.round(
            Math.log((start_cost + total_cost)/2 + start_cost)
          );
    ...
    pgExplain, {
      ...
      DrawJSONPlan: function(container, plan) {
        ...
        var curr_zoom_factor = 1.0,
          zoomArea =$('<div></div>', {
            class: 'pg-explain-zoom-area btn-group', role: 'group',
          }).appendTo(container),
          zoomInBtn = $('<button></button>', {
            class: 'btn pg-explain-zoom-btn badge', title: 'Zoom in',
          }).appendTo(zoomArea).append(
            $('<i></i>', {class: 'fa fa-search-plus'})
          ),
          zoomToNormal = $('<button></button>', {
            class: 'btn pg-explain-zoom-btn badge', title: 'Zoom to original',
          }).appendTo(zoomArea).append(
            $('<i></i>',{class: 'fa fa-arrows-alt'})
          ),
          zoomOutBtn = $('<button></button>', {
            class: 'btn pg-explain-zoom-btn badge', title: 'Zoom out',
          }).appendTo(zoomArea).append($('<i></i>', {
            class: 'fa fa-search-minus',
          })),
          // Main div to be drawn all images on
          planDiv = $('<div></div>', {class: 'pgadmin-explain-container'})
          .appendTo(container),
