define([
  ...
  var MaintenanceModel = Backbone.Model.extend({
    ...
    schema: [
      ...
      },
      {
        ...
        schema:[{
          ...
          control: CustomSwitchControl, label: gettext('FULL'), deps: ['op'],
        },{
          id: 'vacuum_freeze', deps: ['op'], disabled: 'isDisabled',
          control: CustomSwitchControl, label: gettext('FREEZE'), group: gettext('Vacuum'),
        },{
          id: 'vacuum_analyze', deps: ['op'], disabled: 'isDisabled',
          control: CustomSwitchControl, label: gettext('ANALYZE'), group: gettext('Vacuum'),
        }],
      },
      {
        id: 'verbose', group: gettext('Options'), deps: ['op'],
        control: CustomSwitchControl, label: gettext('Verbose Messages'), disabled: 'isDisabled',
      },
    ],
    ...
    isDisabled: function(m) {
      ...
      case 'verbose':
        if (
          'primary_key' in node_info || 'unique_constraint' in node_info ||
            'index' in node_info
        ) {
          if (m.get('op') == 'REINDEX') {
  ...
  pgTools.maintenance = pgTool.extend({
    Init: function() {
       ...
       * Maintenance menu will be enabled only when user select table and
       * database node.
       */
