define('pgadmin.node.synonym', [
  ...
  'pgadmin.browser', 'pgadmin.alertify', 'pgadmin.browser.collection',
], function(gettext, url_for, $, _, pgBrowser, alertify) {

  if (!pgBrowser.Nodes['coll-synonym']) {
    pgBrowser.Nodes['coll-synonym'] = pgBrowser.Collection.extend({
      node: 'synonym', label: gettext('Synonyms'), type: 'coll-synonym',
      columns: ['name', 'owner','is_public_synonym'],
    });
  }

  if (!pgBrowser.Nodes.synonym) {
    pgBrowser.Nodes.synonym = pgBrowser.Node.extend({
      type: 'synonym',
      ...
      model: pgBrowser.Node.Model.extend({
        ...
        schema: [{
          ...
          node: 'schema', filter: function(d) {
            ...
            return d && _.indexOf([
              'pg_catalog', 'sys', 'dbo', 'pgagent', 'information_schema',
              'dbms_job_procedure',
            ], d.label) === -1;
          },
          disabled: function(m) {
        ...
        validate: function() {
          ...
          if (
            _.isUndefined(this.get('synobjschema')) ||
          ...
          if (
            _.isUndefined(this.get('synobjname')) ||
      ...
      canCreate: function(itemData, item, data) {
        ...
        // We do not want to allow create menu by default
        return true;
      },
    });
  }
