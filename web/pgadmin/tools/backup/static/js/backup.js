  ...
  pgBrowser.Backup  = pgTool.extend({
    Init: function() {
      ...
      var backup_supported_nodes = [
        ...
        /**
         * Enable/disable backup menu in tools based on node selected. if
         * selected node is present in supported_nodes, menu will be enabled
         * otherwise disabled.
         *
      ...
      var menu_enabled_server = function(itemData) {
        ...
        return (!_.isUndefined(itemData) && !_.isNull(itemData)) &&
          (('server' === itemData._type) && itemData.connected);
      };
    ...
    start_backup_global_server: function(action, item, params) {
      ...
      if(preference) {
        ...
        alertify.alert(
          gettext(
            'Failed to load preference %(preference)s of module %(module)s.',
            {preference: preference_name, module: module}
          ));
        return;
      }
    ...
    backup_objects: function(action, treeItem) {
      ...
      if(preference) {
        ...
        alertify.alert(
          gettext(
            'Failed to load preference %(preference_name)s of module %(module)s.',
            {preference: preference_name, module: module}
          ));
        return;
      }
      ...
      title = gettext('Backup (%(type)s: %(node)s)', {
        type: node.label, node: data._label,
      });
                ...
                $.ajax({
                  ...
                  error: function(xhr) {
                    try {
                      var err = $.parseJSON(xhr.responseText);
                      alertify.alert(
                        gettext('Backup job failed.'), err.errormsg
                      );
                    } catch (e) {
                      e.stack && console.warn && console.warn(e.stack);
                    }
