define([
  ...
  'underscore.string', 'pgadmin.browser', 'pgadmin.backform',
  'pgadmin.backgrid', 'pgadmin.alertify', 'pgadmin.browser.collection',
  'pgadmin.browser.table.partition.utils',
],
function(gettext, url_for, $, _, S, pgBrowser, Backform, Backgrid, Alertify) {

  if (!pgBrowser.Nodes['coll-partition']) {
    pgBrowser.Nodes['coll-partition'] = pgBrowser.Collection.extend({
      node: 'partition', label: gettext('Partitions'),
  ...
  if (!pgBrowser.Nodes['partition']) {
    pgBrowser.Nodes['partition'] = pgBrowser.Node.extend({
      parent_type: 'table',
      ...
      callbacks: {
        ...
        truncate: function(args, params) {
          ...
          Alertify.confirm(
            ...
            gettext('Are you sure you want to truncate table %(label)s?', {
              label: d._label,
            }),
            function (e) {
              if (e) {
        ...
        reset_table_stats: function(args) {
          ...
          Alertify.confirm(
            ...
            gettext('Are you sure you want to reset the statistics for table %(label)s?', {
              label: d._label,
            }),
            function (e) {
              if (e) {
        ...
        detach_partition: function(args) {
          ...
          Alertify.confirm(
            ...
            gettext('Are you sure you want to detach the partition %(label)s?', {
              label: d._label,
            }),
            function (e) {
              if (e) {
      ...
      model: pgBrowser.Node.Model.extend({
        ...
        },
        {
              ...
              collection.on('change:is_primary_key', function(m) {
                ...
                if(m.get('is_primary_key')) {
                  ...
                  if (_.isUndefined(primary_key.get('oid'))) {
                    ...
                    var primary_key_column_exist = primary_key_column_coll
                ...
                // remove column from primary key.
                  if (primary_key_coll.length > 0) {
                    ...
                    if (removedCols.length > 0) {
                      ...
                      _.each(removedCols, function(m) { m.destroy(); });
          ...
          mode: ['properties', 'create', 'edit'], disabled: 'checkOfType',
          url: 'get_oftype', group: gettext('Advanced'),
                ...
                $.ajax({
                  ...
                  success: function(res) {
                    if (res.success == 1) {
                      ...
                      Alertify.alert(
                        gettext('Error fetching tables to be attached'),
                        res.data.result
                      );
                    }
                  },
