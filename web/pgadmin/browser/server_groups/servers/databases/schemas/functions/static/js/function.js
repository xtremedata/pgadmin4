...
define('pgadmin.node.function', [
  'sources/gettext', 'sources/url_for', 'underscore', 'backbone',
  'pgadmin.backform', 'pgadmin.browser',  'pgadmin.browser.collection',
  'pgadmin.browser.server.privilege',
], function(gettext, url_for, _, Backbone, Backform, pgBrowser) {

  if (!pgBrowser.Nodes['coll-function']) {
    pgBrowser.Nodes['coll-function'] = pgBrowser.Collection.extend({
      node: 'function', label: gettext('Functions'), type: 'coll-function',
    ...
    pgBrowser.Nodes.function = pgBrowser.Node.extend({
      ...
      model: pgBrowser.Node.Model.extend({
        ...
        validate: function() {
          ...
          var errmsg,
            setErrorIfParamEmpty = function(param, msg) {
              ...
              if (
                ...
                this.errorModel.set(param, msg);
                errmsg = msg;
                return true;
              }
              this.errorMode.unset(param);
              return false;
            }.bind(this),
            setErrorForSecLabelError = function() {
              var seclabels = this.get('seclabels');

              if (seclabels) {
                var secLabelsErr, i;
                for (i = 0; i < seclabels.models.length && !secLabelsErr; i++) {
                  secLabelsErr = (seclabels.models[i]).validate.apply(
                    seclabels.models[i]
                  );
                  if (secLabelsErr) {
                    this.errorModel.set('seclabels', secLabelsErr);
                    errmsg = secLabelsErr;
                    return true;
                  }
                }
          ...
          setErrorIfParamEmpty('name', gettext('Name cannot be empty.')) ||
            setErrorIfParamEmpty('funcowner', gettext('Owner cannot be empty.')) ||
            setErrorIfParamEmpty(
              'pronamespace', gettext('Schema cannot be empty.')
            ) ||
            setErrorIfParamEmpty(
              'prorettypename', gettext('Return type cannot be empty.')
            ) ||
            setErrorIfParamEmpty(
              'lanname', gettext('Language cannot be empty.')
            ) || (String(this.get('lanname')) === 'c' ? (
              setErrorIfParamEmpty(
                'probin', gettext('Object File cannot be empty.')
              ) ||
                setErrorIfParamEmpty(
                  'prosrc_c', gettext('Link Symbol cannot be empty.')
                )
            ) : setErrorIfParamEmpty(
              'prosrc',gettext('Code cannot be empty.')
            )) || setErrorForSecLabelError();

          return errmsg;
        },
        isVisible: function() {
          return this.name !== 'sysproc';
        },
