define('pgadmin.node.schema', [
...
], function(gettext, url_for, $, _, Backform, Backgrid, pgBrowser) {

  // Extend the browser's collection class for schema collection
  if (!pgBrowser.Nodes['coll-schema']) {
    pgBrowser.Nodes['coll-schema'] = pgBrowser.Collection.extend({
      node: 'schema', label: gettext('Schemas'), type: 'coll-schema',
  ...
  if (!pgBrowser.Nodes.schema) {
    ...
    var VacuumCollectionControl = Backform.Control.extend({
    ...
    var vacuumVariableCellFunction = function(model) {
      switch(model.get('column_type')) {
      case 'integer':
    ...
    var VacuumTableModel = pgBrowser.Node.Model.extend({
      defaults: {
        name: undefined,
        setting: undefined,
        label:undefined,
    ...
    Backform.VacuumSettingsSchema = [{
      ...
      control: VacuumCollectionControl.extend({
        grid_columns :[
          {
          ...
          },
          {
            ...
            cellFunction: vacuumVariableCellFunction, editable: function(m) {
          ...
          },
          {
            ...
            cellHeaderClasses:'width_percent_30',
            headerCell: Backgrid.Extension.CustomHeaderCell,
            cellFunction: vacuumVariableCellFunction, editable: false,
          },
        ],
      }),
      ...
      model: pgBrowser.Node.Model.extend({
        ...
        validate: function() {
          ...
          if (
            ...
            _.isUndefined(this.get('namespaceowner')) ||
