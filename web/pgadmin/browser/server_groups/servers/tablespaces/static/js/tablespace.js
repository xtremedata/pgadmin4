define('pgadmin.node.tablespace', [
  ...
  'pgadmin.backform', 'pgadmin.browser', 'pgadmin.alertify',
  'pgadmin.browser.collection', 'pgadmin.browser.node.ui',
  'pgadmin.browser.server.privilege',
], function(gettext, url_for, $, _, Backbone, Backform, pgBrowser, alertify) {

  if (pgBrowser.Nodes.tablespace) {
    return pgBrowser.Nodes.tablespace;
  }

  if (!pgBrowser.Nodes['coll-tablespace']) {
    pgBrowser.Nodes['coll-tablespace'] = pgBrowser.Collection.extend({
      node: 'tablespace', label: gettext('Tablespaces'), hasStatistics: true,
      type: 'coll-tablespace', statsPrettifyFields: ['Size'],
