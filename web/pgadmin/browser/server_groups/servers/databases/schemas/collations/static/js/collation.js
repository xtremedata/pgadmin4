define('pgadmin.node.collation', [
  ...
  if (!pgBrowser.Nodes['coll-collation']) {
    pgBrowser.Nodes['coll-collation'] = pgBrowser.Collection.extend({
      node: 'collation', label: gettext('Collations'), type: 'coll-collation',
      columns: ['name', 'owner', 'description'],
    });
  }

  if (!pgBrowser.Nodes.collation) {
    pgBrowser.Nodes.collation = pgBrowser.Node.extend({
      type: 'collation',
      ...
      model: pgBrowser.Node.Model.extend({
        ...
        validate: function() {
          ...
          if (
            _.has(changedAttrs,data.lc_type) &&
              (_.isUndefined(this.get('lc_type')) ||
            String(this.get('lc_type')).replace(/^\s+|\s+$/g, '') === '')
          ) {
            lc_type_flag = true;
          }
          if (
            _.has(changedAttrs,data.copy_collation) &&
              (_.isUndefined(this.get('copy_collation') ||
                String(
                  this.get('copy_collation')
                ).replace(/^\s+|\s+$/g, '') == ''))
          ) {
            copy_coll_flag = true;
          }
