define('pgadmin.node.foreign_server', [
  'sources/gettext', 'jquery', 'underscore', 'pgadmin.backform',
  'pgadmin.browser', 'pgadmin.browser.collection',
  'pgadmin.browser.server.privilege',
], function(gettext, $, _, S, Backform, pgBrowser) {

  // Extend the browser's collection class for foreign server collection
  if (!pgBrowser.Nodes['coll-foreign_server']) {
    pgBrowser.Nodes['coll-foreign_server'] = pgBrowser.Collection.extend({
      node: 'foreign_server', label: gettext('Foreign Servers'),
      type: 'coll-foreign_server', columns: ['name','fsrvowner','description'],
    });
  }
  ...
  if (!pgBrowser.Nodes.foreign_server) {
    ...
    var OptionsModel = pgBrowser.Node.Model.extend({
      ...
      defaults: {fsrvoption: undefined, fsrvvalue: undefined},

      // Defining schema for the Options model
      schema: [{
        id: 'fsrvoption', label: gettext('Options'), type:'text',
        cellHeaderClasses:'width_percent_50', group: null, editable: true,
      },{
        id: 'fsrvvalue', label: gettext('Value'), type: 'text',
        cellHeaderClasses:'width_percent_50', group:null, editable: true,
      }],
    ...
    pgBrowser.Nodes.foreign_server = pgBrowser.Node.extend({
      ...
      model: pgBrowser.Node.Model.extend({
        ...
        schema: [{
          ...
          id: 'fsrvtype', label: gettext('Type'), cell: 'string', type: 'text',
          ...
          id: 'fsrvoptions', label: 'Options', type: 'collection',
          group: gettext('Options'), model: OptionsModel, canAdd: true,
          control: 'unique-col-collection', mode: ['edit', 'create'],
          canDelete: true, uniqueCol : ['fsrvoption'],
          columns: ['fsrvoption','fsrvvalue'],
        }, pgBrowser.SecurityGroupSchema, {
          id: 'fsrvacl', label: 'Privileges', type: 'collection',
          group: 'security',
          model: pgBrowser.Node.PrivilegeRoleModel.extend({privileges: ['U']}),
          control: 'unique-col-collection', mode: ['edit', 'create'],
