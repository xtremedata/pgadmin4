...
define('pgadmin.profiling', [
  ...
  if (pgAdmin.Profiling)
    return;

  var profilingVisible = true,
    cancel_query_url = '',
    terminate_session_url = '',
    is_super_user = false,
    current_user,
    maintenance_database,
    is_server_profiling = false,
    is_database_profiling = false;

  // Custom BackGrid cell, Responsible for cancelling active sessions
  var customProfilingActionCell = Backgrid.Extension.DeleteCell.extend({
    render: function() {
      this.$el.empty();
      var self = this,
  ...
  pgAdmin.Profiling = {
    init: function() {
      ...
      // Bind the Profiling object with the 'object_selected' function
      ...
      // Load the default welcome profiling
      var url = url_for('profiling.index');

      /* Store the chart objects, refresh freq and next refresh time */
      this.chart_store = {};
      this.charts_poller_int_id = -1;

      var profilingPanel = pgBrowser.panels['profiling'].panel;
      if (profilingPanel) {
        var div = profilingPanel.layout().scene().find('.pg-panel-content');
        ...
        if (div) {
          var ajaxHook = function() {
            $.ajax({
              ...
              .fail(function(xhr, error) {
                Alertify.pgNotifier(
                  error, xhr,
                  gettext('An error occurred whilst loading the profiling.'),
                  function(msg) {
                    if(msg === 'CRYPTKEY_SET') {
                      ajaxHook();
                    } else {
                      $(div).html(
                        '<div class="alert alert-danger pg-panel-message" role="alert">' + gettext('An error occurred whilst loading the profiling.') + '</div>'
                      );
                    }
                  }
                );
          ...
          $(div).html(
            '<div class="alert alert-info pg-panel-message" role="alert">' + gettext('Loading profiling...') + '</div>'
          );
          ajaxHook();

          // Cache the current IDs for next time
          $(profilingPanel).data('sid', -1);
          $(profilingPanel).data('did', -1);
        }
      }
    },
    ...
    object_selected: function(item, itemData, node) {
      ...
      if (profilingVisible === false) {
        /*
         * Clear all the interval functions, even when profiling is not
        ...
        var treeHierarchy = node.getTreeNodeHierarchy(item),
          url = NodesProfiling.url(itemData, item, treeHierarchy);
        ...
        if (url === null) {
          ...
          if ('database' in treeHierarchy) {
            ...
            self.sid = treeHierarchy.server._id;
            self.did = -1;
            is_server_profiling = true;
            is_database_profiling = false;
            url += self.sid;
            cancel_query_url += self.sid + '/';
            terminate_session_url += self.sid + '/';
          } else {
            is_server_profiling = is_database_profiling = false;
          }
        } else {
          is_server_profiling = is_database_profiling = false;
        }
        ...
        var profilingPanel = pgBrowser.panels['profiling'].panel;
        if (profilingPanel) {
          var div = profilingPanel.layout().scene().find(
            '.pg-panel-content'
          );
          ...
          if (div) {
            if (itemData.connected || _.isUndefined(itemData.connected)) {
              ...
              if (
                url !== $(profilingPanel).data('profiling_url') || (
                  url === $(profilingPanel).data('profiling_url') &&
                  $(profilingPanel).data('server_status') == false
                )
              ) {
                $(div).empty();

                /* Clear all the charts previous profilings */
                self.clearChartFromStore();
                ...
                ajaxHook();
                $(profilingPanel).data('server_status', true);
              }
            } else {
              $(div).empty();
              if (
              ...
              $(div).html(
                '<div class="alert alert-info pg-panel-message" role="alert">' + gettext('Please connect to the selected server to view the profiling.') + '</div>'
              );
              $(profilingPanel).data('server_status', false);
            }
            // Cache the current IDs for next time
            $(profilingPanel).data('profiling_url', url);
          }
        }
      }
    },
    ...
    getStatsUrl: function(sid=-1, did=-1, chart_names=[]) {
      let base_url = url_for('profiling.profiling_stats');
      base_url += '/' + sid;
      base_url += (did > 0) ? ('/' + did) : '';
    ...
    render_grid_data: function(container) {
      ...
      data.fetch({
        ...
        error: function(model, xhr) {
          ...
          // Try again
          setTimeout(function() {
            pgAdmin.Profiling.render_grid_data(container, data);
          }, 5000);
        },
      });
    },
    ...
    init_profiling: function() {
      ...
      pgBrowser.onPreferencesChange('profilings', function() {
        self.reflectPreferences();
      });
    ...
    reflectPreferences: function() {
      ...
      if(self.preferences) {
        ...
        self.preferences = pgWindow.default.pgAdmin.Browser.get_preferences_for_module('profilings');
      }
      else {
        self.preferences = pgWindow.default.pgAdmin.Browser.get_preferences_for_module('profilings');
        self.old_preferences = self.preferences;
      ...
      if(is_server_profiling || is_database_profiling) {
        /* Common things can come here */
        var div_sessions = $('.profiling-container').find('#graph-sessions')[0];
        var div_tps = $('.profiling-container').find('#graph-tps')[0];
        var div_ti = $('.profiling-container').find('#graph-ti')[0];
        var div_to = $('.profiling-container').find('#graph-to')[0];
        var div_bio = $('.profiling-container').find('#graph-bio')[0];
        var options_line = {
          parseFloat: false,
          xaxis: {
            min: 100,
        ...
        if(self.preferences.show_graphs && $('#profiling-graphs').hasClass('profiling-hidden')) {
          $('#profiling-graphs').removeClass('profiling-hidden');
        }
        else if(!self.preferences.show_graphs) {
          $('#profiling-graphs').addClass('profiling-hidden');
          self.clearChartFromStore();
        }

        if (self.preferences.show_activity && $('#profiling-activity').hasClass('profiling-hidden')) {
          $('#profiling-activity').removeClass('profiling-hidden');
        }
        else if(!self.preferences.show_activity) {
          $('#profiling-activity').addClass('profiling-hidden');
        }

        if(self.preferences.show_graphs) {
          // Render the graphs
          pgAdmin.Profiling.renderCharts([{
        ...
        if(!self.preferences.show_graphs && !self.preferences.show_activity) {
          $('#profiling-none-show').removeClass('profiling-hidden');
        }
        else {
          $('#profiling-none-show').addClass('profiling-hidden');
        }

        /* Profiling specific preferences can be updated in the
         * appropriate functions
         */
        if(is_server_profiling) {
          self.reflectPreferencesServer();
        }
        else if(is_database_profiling) {
    ...
    reflectPreferencesServer: function() {
      var self = this;
      var $profilingContainer = $('.profiling-container');
      var div_server_activity = $profilingContainer.find('#server_activity').get(0);
      var div_server_locks = $profilingContainer.find('#server_locks').get(0);
      var div_server_prepared = $profilingContainer.find('#server_prepared').get(0);
      var div_server_config = $profilingContainer.find('#server_config').get(0);
      ...
      if (self.preferences.show_activity) {
        ...
        server_activity_columns.unshift({
          ...
          label: '',
          cell: customProfilingActionCell,
          cell_action: 'cancel',
          editable: false,
          cell_priority: -1,
          canDeleteRow: pgAdmin.Profiling.can_take_action,
          postgres_version: self.version,
        });
        ...
        pgAdmin.Profiling.render_grid(
          div_server_activity, url_for('profiling.activity'), server_activity_columns
        );
        pgAdmin.Profiling.render_grid(
          div_server_locks, url_for('profiling.locks'), server_locks_columns
        );
        pgAdmin.Profiling.render_grid(
          div_server_prepared, url_for('profiling.prepared'), server_prepared_columns
        );
        pgAdmin.Profiling.render_grid(
          div_server_config, url_for('profiling.config'), server_config_columns
        );

        pgAdmin.Profiling.render_grid_data(div_server_activity);
        ...
        $('a[data-toggle="tab"]').on('shown.bs.tab', function(e) {
          ...
          pgAdmin.Profiling.render_grid_data(currGrid);
        });

        $('#btn_refresh').off('click').on('click', () => {
          let currGrid = tab_grid_map[$('#profiling-activity .nav-tabs .active').attr('aria-controls')];
          pgAdmin.Profiling.render_grid_data(currGrid);
        });
      }
    },
    reflectPreferencesDatabase: function() {
      ...
      if (self.preferences.show_activity) {
        ...
        // Render the tabs, but only get data for the activity tab for now
        pgAdmin.Profiling.render_grid(
          div_database_activity, url_for('profiling.activity'), database_activity_columns
        );
        pgAdmin.Profiling.render_grid(
          div_database_locks, url_for('profiling.locks'), database_locks_columns
        );
        pgAdmin.Profiling.render_grid(
          div_database_prepared, url_for('profiling.prepared'), database_prepared_columns
        );

        pgAdmin.Profiling.render_grid_data(div_database_activity);
    ...
    can_take_action: function(m) {
      ...
      // With PG10, We have background process showing on profiling
