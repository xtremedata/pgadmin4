...
define('pgadmin.node.foreign_table', [
  ...
  if (!pgBrowser.Nodes['coll-foreign_table']) {
    pgBrowser.Nodes['coll-foreign_table'] = pgBrowser.Collection.extend({
      node: 'foreign_table', label: gettext('Foreign Tables'),
      type: 'coll-foreign_table', columns: ['name', 'owner', 'description'],
    });
  }
  ...
  Backgrid.IntegerCell.extend({
    initialize: function() {
      ...
      Backgrid.Extension.DependentCell.prototype.initialize.apply(
        this, arguments
      );
    },
    dependentChanged: function () {
      ...
      var is_editable = _.isFunction(editable) ? !!editable.apply(
        column, [model]
      ) : !!editable;
  ...
  var ColumnOptionsModel = pgBrowser.Node.Model.extend({
    ...
    schema: [{
      id: 'option', label:'Option', type:'text', editable: true,
    ...
    pgBrowser.Nodes['foreign_table'] = pgBrowser.Node.extend({
      ...
      model: pgBrowser.Node.Model.extend({
        ...
        },
        {
          ...
          type: 'collection', group: 'security',
