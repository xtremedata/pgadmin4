define('pgadmin.node.index', [
  ...
  if (!pgBrowser.Nodes['coll-index']) {
    pgBrowser.Nodes['coll-index'] = pgBrowser.Collection.extend({
      node: 'index', label: gettext('Indexes'), type: 'coll-index',
      getTreeNodeHierarchy: pgBrowser.tableChildTreeNodeHierarchy,
      sqlAlterHelp: 'sql-alterindex.html', sqlCreateHelp: 'sql-createindex.html',
      dialogHelp: url_for('help.static', {'filename': 'index_dialog.html'}),
      columns: ['name', 'description'], hasStatistics: true,
  ...
  var ColumnModel = pgBrowser.Node.Model.extend({
    defaults: {
      colname: undefined,
  ...
  if (!pgBrowser.Nodes.index) {
    pgBrowser.Nodes.index = pgBrowser.Node.extend({
      getTreeNodeHierarchy: pgBrowser.tableChildTreeNodeHierarchy,
